...
import sqlite3, os, io, random, string, json
from datetime import datetime, timedelta
from openpyxl import Workbook
from openpyxl.styles import Font, Alignment, PatternFill, Border, Side
from openpyxl.utils import get_column_letter

# qrcode é opcional — se não instalado, o QR é gerado pelo browser via jsQR
try:
    import qrcode as _qrcode_lib
    QR_DISPONIVEL = True
except ImportError:
    QR_DISPONIVEL = False
    print("[Apollo] Módulo 'qrcode' não encontrado — QR Code será gerado pelo navegador.")
...
def cadastro_solicitar():
    ...
    if len(senha) < 6:
        return resp_erro('Senha deve ter pelo menos 6 caracteres.')
...
def registrar_movimentacao():
    ...
    c.execute(
        'INSERT INTO movimentacoes (codigo_produto, nome_produto, categoria, tipo, quantidade, estoque_apos, data, operador, registrado_em) VALUES (?,?,?,?,?,?,?,?,?)',
        (codigo, nome, cat, tipo, qtd, novo_estoque, data, session.get('usuario',''), datetime.now().isoformat())
    )

    # Gera QR Code em base64
    qr_b64 = gerar_qr_base64(codigo, nome, cat, tipo, qtd, novo_estoque, minimo, data)
...
def movimentacoes_hoje():
    ...
    conn = get_db()
    rows = conn.execute(
        'SELECT * FROM movimentacoes WHERE data=? ORDER BY id DESC', (hoje,)
    ).fetchall()
    conn.close()
    return resp_ok([dict(r) for r in rows])
...
def gerar_qr_base64(codigo, nome, cat, tipo, qtd, atual, minimo, data):
    ...
    Tenta usar 'qrcode' (se instalado); caso contrário usa fallback
    com geração via matriz Reed–Solomon simplificada usando apenas
    a stdlib (struct + zlib) para criar um PNG válido com o texto.
    O frontend (jsQR) consegue ler ambos.
    """
    try:
    ...
    txt = (f"APOLLO LOGISTICA\nCodigo: {codigo}\nProduto: {nome}\n"
           f"Categoria: {cat}\nTipo: {tipo.upper()}\nQuantidade: {qtd}\n"
           f"Estoque atual: {atual}\nEstoque minimo: {minimo}\nData: {dt}")

    # Tenta qrcode externo
    if QR_DISPONIVEL:
        try:
    ...
    # Fallback: retorna string vazia — o frontend gera via QRCode.js
    return ''
...
def gerar_qr_endpoint():
    ...
    b64 = gerar_qr_base64(
        ...
        d.get('tipo',''), d.get('quantidade',0),
        d.get('estoque_atual',0), d.get('estoque_minimo',0), d.get('data','')
    )
