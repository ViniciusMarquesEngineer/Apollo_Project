...
import sqlite3, os, io, random, string, json, base64, threading
...
try:
    ...
    print("[Apollo] cv2/pyzbar não encontrados — instale: pip install opencv-python pyzbar")
...
app.secret_key = 'apollo_secret_2026_piec'
...
# APAGAR ESSE PRINT DEPOIS (USADO PARA FAZER OS TESTES)
...
# ═══════════════════════════════════════════════════════════
#  LEITURA DE QR CODE via OpenCV + pyzbar (camera local)
# ═══════════════════════════════════════════════════════════
_scan_result = {'dados': None, 'erro': None, 'pronto': True}
_scan_lock   = threading.Lock()


def _thread_scan_camera():
    """
    Lê QR Code pela câmera usando OpenCV + pyzbar.
    Opera em modo headless (sem janela cv2.imshow) para compatibilidade
    com servidores Windows/Linux sem display.
    A cada frame capturado tenta decodificar com pyzbar; timeout de 30s.
    """
    global _scan_result
    if not CV2_DISPONIVEL:
        with _scan_lock:
            _scan_result = {'dados': None,
                            'erro': 'cv2/pyzbar nao instalados. Execute: pip install opencv-python pyzbar',
                            'pronto': True}
        return

    # Tenta abrir câmera — índices 0, 1 e 2
    cap = None
    for idx in (0, 1, 2):
        c = cv2.VideoCapture(idx, cv2.CAP_DSHOW if hasattr(cv2, 'CAP_DSHOW') else 0)
        if c.isOpened():
            cap = c
            break
        c.release()
    ...
    if cap is None:
        with _scan_lock:
            _scan_result = {'dados': None,
                            'erro': 'Camera nao encontrada. Verifique se ela esta conectada e nao esta em uso.',
                            'pronto': True}
        return

    cap.set(cv2.CAP_PROP_FRAME_WIDTH,  1280)
    cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 720)
    cap.set(cv2.CAP_PROP_BUFFERSIZE, 1)   # descarta frames antigos

    deadline = datetime.now().timestamp() + 30
    found    = False

    # Aquece câmera — descarta primeiros frames que costumam vir escuros
    for _ in range(5):
        cap.read()
    ...
    while datetime.now().timestamp() < deadline:
        ...
        # pyzbar decodifica direto no frame numpy (BGR do OpenCV)
        decoded = pyzbar.decode(frame)
        if decoded:
            txt = decoded[0].data.decode('utf-8', errors='replace').strip()
            if txt:
                found = True
                with _scan_lock:
                    _scan_result = {'dados': txt, 'erro': None, 'pronto': True}
                break
    ...
    if not found:
        with _scan_lock:
            _scan_result = {'dados': None,
                            'erro': 'Nenhum QR Code detectado em 30 segundos. Tente novamente.',
                            'pronto': True}


@app.route('/api/qr/iniciar-scan', methods=['POST'])
def qr_iniciar_scan():
    """Dispara a thread de leitura de QR Code via camera OpenCV."""
    if not usuario_logado():
        return resp_erro('Nao autenticado.', 401)
    if not CV2_DISPONIVEL:
        return resp_erro('OpenCV/pyzbar nao instalados. Execute: pip install opencv-python pyzbar')
    global _scan_result
    with _scan_lock:
        if not _scan_result.get('pronto', True):
            return resp_erro('Leitura ja em andamento.')
        _scan_result = {'dados': None, 'erro': None, 'pronto': False}
    t = threading.Thread(target=_thread_scan_camera, daemon=True)
    t.start()
    return resp_ok({'msg': 'Camera iniciada. Aguarde o resultado.'})


@app.route('/api/qr/resultado', methods=['GET'])
def qr_resultado():
    """Polling: retorna o resultado da ultima leitura de camera."""
    if not usuario_logado():
        return resp_erro('Nao autenticado.', 401)
    with _scan_lock:
        r = dict(_scan_result)
    if not r.get('pronto'):
        return resp_ok({'pronto': False})
    if r['erro']:
        return resp_erro(r['erro'])
    return resp_ok({'pronto': True, 'texto': r['dados']})
